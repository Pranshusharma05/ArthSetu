    ...
    return (
      <div className="max-w-3xl mx-auto mt-8 mb-24 px-4">
        <div className="text-center mb-10">
          <h2 className="text-2xl md:text-3xl font-extrabold text-primary mb-3">Your Scheme Results</h2>
          <p className="text-[14px] text-text-muted">Based on your profile, ArthSetu found the following verified Government schemes.</p>
        </div>
        {schemeResults.recommended && schemeResults.recommended.length > 0 && (
          <div className="space-y-4 mb-8">
            <h3 className="text-[16px] font-bold text-primary mb-4">Recommended Schemes ({schemeResults.recommended.length})</h3>
            {schemeResults.recommended.map((scheme: any) => (
              <div key={scheme.id} className="bg-white rounded-[16px] border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div><div className="font-bold text-primary text-[16px] mb-1">{scheme.name}</div><div className="text-[12px] text-text-muted">{scheme.officialSource}</div></div>
                    <span className="bg-soft-teal text-secondary text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide flex-shrink-0">Eligible</span>
                  </div>
                  <p className="text-[13px] text-text-muted leading-relaxed mb-4">{scheme.description}</p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {scheme.applicationRoute && <span className="text-[11px] font-bold bg-primary/5 text-primary px-3 py-1 rounded-full">{scheme.applicationRoute}</span>}
                    {scheme.benefitType && <span className="text-[11px] font-bold bg-gray-100 text-text-muted px-3 py-1 rounded-full">{scheme.benefitType}</span>}
                  </div>
                  <div className="text-[11px] text-text-muted flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-secondary" /><span>Verified · Last checked: {scheme.lastVerified || 'Recently'}</span></div>
                </div>
                <div className="border-t border-gray-50">
                  <button onClick={() => toggleExpanded(scheme.id, 'details')} className="w-full px-5 py-3 flex items-center justify-between text-[13px] font-semibold text-text-muted hover:bg-gray-50 transition-colors">
                  ...
                  {expandedDetails[scheme.id] && (
                    <div className="px-5 pb-4 bg-gray-50/50 space-y-2">
                      {scheme.ruleComparisons?.filter((r: any) => r.status === 'Matched').map((evalItem: any, idx: number) => (
                        <div key={idx} className="flex items-start justify-between gap-3 py-2 border-b border-gray-100 last:border-0">
                          <div className="flex-1 min-w-0">
                            <div className="text-[12px] font-bold text-text-main mb-0.5">{evalItem.ruleName}</div>
                            <div className="text-[12px] text-text-muted"><span className="font-semibold text-text-main">Your value:</span> {evalItem.userValue}</div>
                            <div className="text-[12px] text-text-muted"><span className="font-semibold text-text-main">Scheme condition:</span> {evalItem.schemeCondition}</div>
                          </div>
                          <div className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex-shrink-0 ${evalItem.status === 'Matched' ? 'bg-soft-teal text-secondary' : evalItem.status === 'Failed' ? 'bg-red-50 text-red-600' : 'bg-yellow-50 text-yellow-700'}`}>{evalItem.status}</div>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {schemeResults.moreInfoNeeded && schemeResults.moreInfoNeeded.length > 0 && (
          <div className="border border-gray-100 rounded-xl overflow-hidden bg-white mt-8">
            <button onClick={() => setExpandedMoreInfo(!expandedMoreInfo)} className="w-full px-5 py-4 flex items-center justify-between text-[14px] font-bold text-text-main hover:bg-gray-50 transition-colors">
              <span>More Information Needed</span><ChevronDown className={`w-5 h-5 text-text-muted transition-transform ${expandedMoreInfo ? 'rotate-180' : ''}`} />
            </button>
            {expandedMoreInfo && (
              <div className="px-5 pb-5 pt-1 border-t border-gray-50 space-y-4">
                {schemeResults.moreInfoNeeded.map((scheme: any) => (
                  <div key={scheme.id}>
                    ...
                    <div className="text-[13px] font-medium text-orange-500 mb-1">Needs: {scheme.ruleComparisons?.filter((r: any) => r.status === 'Missing').map((r: any) => r.ruleName).join(', ')}</div>
        ...
        {schemeResults.notEligible && schemeResults.notEligible.length > 0 && (
          <div className="border border-gray-100 rounded-xl overflow-hidden bg-white mt-8 mb-8">
            <button onClick={() => setExpandedAlternative(!expandedAlternative)} className="w-full px-5 py-4 flex items-center justify-between text-[14px] font-bold text-text-main hover:bg-gray-50 transition-colors">
              <span>Schemes You Do Not Qualify For</span><ChevronDown className={`w-5 h-5 text-text-muted transition-transform ${expandedAlternative ? 'rotate-180' : ''}`} />
            </button>
            {expandedAlternative && (
              <div className="px-5 pb-5 pt-1 border-t border-gray-50 space-y-4">
                {schemeResults.notEligible.map((scheme: any) => (
                  <div key={scheme.id} className="border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                    <div className="font-bold text-primary text-[14px] mb-1">{scheme.name}</div>
                    <button onClick={() => toggleExpanded(scheme.id, 'notEligible')} className="flex items-center gap-1.5 text-[12px] font-bold text-accent hover:text-primary transition-colors mt-2 mb-3">
                      <span>Why this scheme did not match</span><ChevronDown className={`w-3 h-3 transition-transform ${expandedNotEligible[scheme.id] ? 'rotate-180' : ''}`} />
                    </button>
                            ...
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                                  <div className="text-[12px]"><span className="text-text-muted uppercase tracking-wide text-[10px] block mb-0.5">Your Profile</span><span className="font-medium text-text-main">{rule.userValue}</span></div>
                                  <div className="text-[12px]"><span className="text-text-muted uppercase tracking-wide text-[10px] block mb-0.5">Scheme Requirement</span><span className="font-medium text-text-main">{rule.schemeCondition}</span></div>
                                </div>
                              </div>
                            </div>
                          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-8 mb-16">
          <button onClick={() => { setStep('intro'); setStepHistory([]); }} className="w-full sm:w-auto flex-1 flex items-center justify-center bg-white text-primary border border-primary/20 px-8 py-4 rounded-[12px] font-bold text-[16px] hover:border-primary hover:-translate-y-0.5 hover:shadow-sm transition-all shadow-sm">Start New Search</button>
        </div>
      </div>
    );
  };
      ...
      {step !== 'intro' && step !== 'processing' && step !== 'result' && (
        <div className="max-w-2xl mx-auto px-4 mb-12">
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="text-[12px] font-bold text-text-muted uppercase tracking-wider">Step {getStepNumber()} of 6</span>
          ...
          <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-secondary transition-all duration-500 ease-out rounded-full" style={{ width: `${(getStepNumber() / 6) * 100}%` }}></div>
          </div>
        </div>
      )}
